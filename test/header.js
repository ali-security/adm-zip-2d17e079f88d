...
describe("headers", () => {
    describe("main-header", () => {
        ...
        it("reads back the zip64 end record it wrote", () => {
            const mainh = new mainHeader();
            mainh.totalEntries = 0x10000;
            mainh.size = 123;
            mainh.offset = 456;

            const read = new mainHeader();
            read.loadFromBinary(mainh.toBinary().slice(0, Constants.ZIP64HDR));

            expect(read.diskEntries).to.equal(0x10000);
            expect(read.totalEntries).to.equal(0x10000);
            // the central directory size lives at ZIP64SIZB - ZIP64SIZE holds the size
            // of the zip64 record itself and must not be read as the directory size
            expect(read.size).to.equal(123);
            expect(read.offset).to.equal(456);
        });
